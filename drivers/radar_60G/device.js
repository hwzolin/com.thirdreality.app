...
class Radar_60G extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    if (
      !this.getStoreValue("colorClusterConfigured") &&
  ...
  onOccupancySensingChangeNotification({ occupied }) {
    ...
    this.setCapabilityValue("alarm_occupancy", occupied).catch((error) =>
      this.log(error),
    );
  }
  ...
  setLuminanceValue(value) {
    ...
    this.setCapabilityValue("measure_luminance", zigbee_illuminance).catch(
  ...
  get levelControlCluster() {
    const levelControlClusterEndpoint = this.getClusterEndpoint(
      CLUSTER.LEVEL_CONTROL,
    );
    if (levelControlClusterEndpoint === null)
  ...
  async changeOnOff(onoff) {
    ...
    return this.onOffCluster[onoff ? "setOn" : "setOff"]().then(
      async (result) => {
        if (onoff === false) {
          ...
          wait(1000)
            .then(async () => {
              ...
              const { currentLevel } =
                await this.levelControlCluster.readAttributes([CURRENT_LEVEL]);
              this.debug("changeOnOff() →", onoff, { currentLevel });
              // Always set dim to 0.01 or higher since bulb is turned on
              await this.setCapabilityValue(
                "dim",
                Math.max(0.01, currentLevel / MAX_DIM),
              ).catch(this.error);
            })
            .catch((err) => {
              this.error(
                "Error: could not update dim capability value after `onoff` change",
                err,
              );
            });
        }
  ...
  async changeDimLevel(dim, opts = {}) {
    ...
    return this.levelControlCluster
  ...
  async changeColorTemperature(temperature, opts = {}) {
    ...
    if (this.supportsColorTemperature) {
      ...
      this.debug(
        `changeColorTemperature() → ${temperature} →`,
    ...
    const { hue, saturation, value } =
      mapTemperatureToHueSaturation(temperature);
  ...
  async registerColorCapabilities({ zclNode }) {
    ...
    this.registerMultipleCapabilities(
      ...
      (valueObj, optsObj) => {
        ...
        const lightSaturationChanged =
          typeof valueObj.light_saturation === "number";
        const lightTemperatureChanged =
          typeof valueObj.light_temperature === "number";
        ...
        if (
          lightHueChanged ||
          lightSaturationChanged ||
          (lightModeChanged && valueObj.light_mode === "color")
        ) {
          return this.changeColor(
            ...
            if (err && err.message && err.message.includes("FAILURE")) {
              throw new Error(
                "Make sure the device is turned on before changing its color.",
              );
            }
            throw err;
          });
        ...
        if (
          ...
          return this.changeColorTemperature(valueObj.light_temperature, {
            ...
            if (err && err.message && err.message.includes("FAILURE")) {
              throw new Error(
                "Make sure the device is turned on before changing its color temperature.",
              );
            }
            throw err;
          });
  ...
  async readColorControlAttributes() {
    ...
    return this.colorControlCluster
      .readAttributes([
        "colorCapabilities",
        "colorTemperatureMireds",
      ...
      .then(
        async ({
          ...
          this._supportsHueAndSaturationOption =
            colorCapabilities.hueAndSaturation;
  ...
  async changeColor({ hue, saturation, value }, opts = {}) {
    ...
    if (this.supportsHueAndSaturation) {
      ...
      this.debug(
        "changeColor() → hue and saturation",
        moveToHueAndSaturationCommand,
      );
      return this.colorControlCluster.moveToHueAndSaturation(
        moveToHueAndSaturationCommand,
      );
    }
  ...
  async onEndDeviceAnnounce() {
    ...
    try {
      ...
      const { currentLevel } = await levelControlCluster.readAttributes([
        "currentLevel",
      ]);
      this.setCapabilityValue(
        "dim",
        limitValue(currentLevel / 254, 0, 1),
      ).catch(this.error);
      this.setCapabilityValue(
        "onoff",
        limitValue(currentLevel > 0, 0, 1),
      ).catch(this.error);
    } catch (err) {
    ...
    try {
      ...
      this.log(
        "onEndDeviceAnnounce → read color control attributes",
        colorControlAttributes,
      );
    } catch (err) {
      this.error(
        "onEndDeviceAnnounce → Error: failed to read color control attributes",
        err,
      );
      return;
    }

    const { currentSaturation, currentHue, colorMode, colorTemperatureMireds } =
      colorControlAttributes;

    // If device supports hue and saturation fetch it and update the capability values
    if (
      this.supportsHueAndSaturation &&
      typeof currentHue === "number" &&
      typeof currentSaturation === "number"
    ) {
      await this.setCapabilityValue(
        "light_hue",
        limitValue(currentHue / MAX_HUE, 0, 1),
      ).catch(this.error);
      await this.setCapabilityValue(
        "light_saturation",
        limitValue(currentSaturation / MAX_SATURATION, 0, 1),
      ).catch(this.error);
    }
    ...
    if (this.hasCapability("light_mode")) {
      await this.setCapabilityValue(
        "light_mode",
        colorMode === "colorTemperatureMireds" ? "temperature" : "color",
      ).catch(this.error);
    }
    ...
    if (
      ...
      typeof colorTemperatureMireds === "number"
    ) {
      await this.setCapabilityValue(
        "light_temperature",
        mapValueRange(
    ...
    if (changedKeys == "start_up_on_off") {
      if (newSettings[changedKeys] == "off") {
        this.zclNode.endpoints[1].clusters["onOff"]
          .writeAttributes({ startUpOnOff: 0 })
          .catch((err) => {
            this.error(err);
          });
        ...
        this.zclNode.endpoints[1].clusters["onOff"]
          .writeAttributes({ startUpOnOff: 1 })
          .catch((err) => {
            this.error(err);
          });
        ...
        this.zclNode.endpoints[1].clusters["onOff"]
          .writeAttributes({ startUpOnOff: 2 })
          .catch((err) => {
            this.error(err);
          });
        ...
        this.zclNode.endpoints[1].clusters["onOff"]
          .writeAttributes({ startUpOnOff: 255 })
          .catch((err) => {
            this.error(err);
          });
